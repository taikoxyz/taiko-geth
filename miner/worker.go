...
type generateParams struct {
	...
	baseFeePerGas     *big.Int // CHANGE(taiko): used by legacy Taiko block building.
...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	return miner.generateWorkWithContext(context.Background(), genParam, witness)
}

// generateWorkWithContext generates a sealing block based on the given parameters.
func (miner *Miner) generateWorkWithContext(ctx context.Context, genParam *generateParams, witness bool) (result *newPayloadResult) {
	...
	work, err := miner.prepareWorkWithContext(ctx, genParam, witness)
	if err != nil {
	...
	if !genParam.noTxs {
		...
		if genParam.forceOverrides && len(genParam.overrideTxs) > 0 {
			for _, tx := range genParam.overrideTxs {
				...
				if err := miner.commitTransactionWithContext(ctx, work, tx); err != nil {
			...
			err := miner.fillTransactionsWithContext(ctx, interrupt, work)
...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	return miner.prepareWorkWithContext(context.Background(), genParams, witness)
}

func (miner *Miner) prepareWorkWithContext(ctx context.Context, genParams *generateParams, witness bool) (result *environment, err error) {
...
func (miner *Miner) commitTransaction(env *environment, tx *types.Transaction) error {
	return miner.commitTransactionWithContext(context.Background(), env, tx)
}

func (miner *Miner) commitTransactionWithContext(ctx context.Context, env *environment, tx *types.Transaction) (err error) {
...
func (miner *Miner) commitTransactionsWithContext(ctx context.Context, env *environment, plainTxs, blobTxs *transactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	for {
		...
		err := miner.commitTransactionWithContext(ctx, env, tx)
		switch {
...
func (miner *Miner) fillTransactions(interrupt *atomic.Int32, env *environment) error {
	return miner.fillTransactionsWithContext(context.Background(), interrupt, env)
}

func (miner *Miner) fillTransactionsWithContext(ctx context.Context, interrupt *atomic.Int32, env *environment) (err error) {
	...
	if len(prioPlainTxs) > 0 || len(prioBlobTxs) > 0 {
		...
		if err := miner.commitTransactionsWithContext(ctx, env, plainTxs, blobTxs, interrupt); err != nil {
			return err
		}
	}
