...
var (
	legacyTestVerkleChainConfig = &params.ChainConfig{
...
func TestLegacyProcessVerkle(t *testing.T) {
	var (
		...
		signer                             = types.LatestSigner(legacyTestVerkleChainConfig)
		testKey, _                         = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		bcdb                               = rawdb.NewMemoryDatabase() // Database for the blockchain
		coinbase                           = common.HexToAddress("0x71562b71999873DB5b286dF957af199Ec94617F7")
		gspec                              = &Genesis{
			Config: legacyTestVerkleChainConfig,
			Alloc: GenesisAlloc{
				coinbase: {
...
func TestLegacyProcessParentBlockHash(t *testing.T) {
	...
	checkBlockHashes := func(statedb *state.StateDB, isVerkle bool) {
		...
		for i := 1; i <= num; i++ {
			...
			if isVerkle {
				chainConfig = legacyTestVerkleChainConfig
			}
		...
		for i := 0; i < num; i++ {
			have, want := legacyGetContractStoredBlockHash(statedb, uint64(i), isVerkle), common.Hash{byte(i + 1)}
			if have != want {
...
func legacyGetContractStoredBlockHash(statedb *state.StateDB, number uint64, isVerkle bool) common.Hash {
