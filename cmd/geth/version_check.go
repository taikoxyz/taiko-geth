...
// verifySignature checks that sigdata is a valid signature of data for one of
// the trusted geth public keys.
func verifySignature(pubkeys []string, data, sigdata []byte) error {
	...
	if ok, err := key.Verify(data, sig); !ok || err != nil {
		log.Info("Verification failed", "keyid", keyID(key.KeyId), "error", err)
		return errors.New("signature could not be verified")
	}
	return nil
}

// keyID turns a binary minisign key ID into a hex string. Note that key IDs
// are printed in reverse byte order.
