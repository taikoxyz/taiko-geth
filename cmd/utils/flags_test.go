...
func TestSetEthConfigPreservesExplicitNetworkIDForPresetNetworks(t *testing.T) {
	t.Parallel()
	...
	if err := set.Set(MainnetFlag.Name, "true"); err != nil {
		t.Fatalf("failed to set %q: %v", MainnetFlag.Name, err)
	}
	if err := set.Set(NetworkIdFlag.Name, "12345"); err != nil {
		t.Fatalf("failed to set %q: %v", NetworkIdFlag.Name, err)
	}
	ctx := cli.NewContext(app, set, nil)
	...
	if cfg.NetworkId != 12345 {
		t.Fatalf("network ID mismatch: have %d want %d", cfg.NetworkId, uint64(12345))
	}
}
