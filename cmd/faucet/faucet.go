...
var (
	...
	snæfellsjökullFlag = flag.Bool("snaefellsjokull", false, "Initializes the faucet with Taiko Snæfellsjökull network config")
...
func main() {
	...
	genesis, err := getGenesis(*genesisFlag, *goerliFlag, *rinkebyFlag, *sepoliaFlag, *alpha1Flag, *alpha2Flag, *snæfellsjökullFlag)
	if err != nil {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	go f.beaconSyncLoop(*alpha1Flag, *alpha2Flag, *snæfellsjökullFlag)
...
func (f *faucet) beaconSyncLoop(alpha1Flag bool, alpha2Flag bool, snæfellsjökullFlag bool) {
	var (
		c   *ethclient.Client
		err error
	)

	if alpha1Flag || alpha2Flag || snæfellsjökullFlag {
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			tx := types.NewTransaction(f.nonce+uint64(len(f.reqs)), address, amount, 21000, new(big.Int).SetUint64(f.price.Uint64()*3), nil)
...
func getGenesis(genesisFlag string, goerliFlag bool, rinkebyFlag bool, sepoliaFlag bool, alpha1Flag bool, alpha2Flag bool, snæfellsjökullFlag bool) (*core.Genesis, error) {
	switch {
	case genesisFlag != "":
	...
	case snæfellsjökullFlag:
		return core.TaikoGenesisBlock(167003), nil
	default:
