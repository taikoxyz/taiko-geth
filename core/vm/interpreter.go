...
type Config struct {
	...
	ZkGasMeter *ZkGasMeter // CHANGE(taiko): per-opcode zk gas metering when non-nil (Unzen fork)
}
...
func (evm *EVM) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for {
		...
		if operation.dynamicGas != nil {
			gasAfterStatic := contract.Gas.RegularGas
		...
		if evm.zkGasTracker != nil && evm.zkGasErr == nil {
			if zkErr := evm.zkGasTracker.FinishAndCharge(evm.depth, zkGasStepGasAfter(op, err, gasBefore, contract.Gas.RegularGas)); zkErr != nil {
				evm.setZkGasErr()
				return nil, ErrZkGasLimitExceeded
			}
		}
